    ...
    >
      ...
      <div className="flex items-center justify-between border-b-2 border-black pb-1 mb-1">
        <div>
          ...
          <span className="text-[8px] font-bold text-gray-700 block mt-0.5">
            Manutenção Apple & Android
          </span>
        </div>

        <div className="text-right">
          <span className="font-black text-[15px] bg-black text-white px-2 py-0.5 rounded-sm block">
            O.S. #{os.numero_os}
          </span>
        </div>
      </div>
      ...
      {/* Main Grid Body */}
      <div className="flex items-start justify-between gap-1 flex-1 min-h-0 py-0.5">
        ...
        <div className="flex-1 min-w-0 space-y-1 text-[10px] leading-tight">
          ...
          <div className="truncate flex items-center gap-1">
            ...
            <span className="font-black text-[12px] bg-gray-200 border border-gray-400 px-1.5 py-0.2 rounded-sm text-black">
              {os.senha_aparelho || 'SEM SENHA'}
            </span>
            {os.buscar_iphone_desativado && (
              <span className="text-[8px] font-black bg-black text-white px-1 py-0.2 rounded-xs">
                F.iPh OFF
              </span>
            )}
          </div>
        ...
        {/* QR Code Section */}
        <div className="flex flex-col items-center justify-center shrink-0 border-l border-black pl-1.5 ml-0.5">
          <QRCodeSVG value={publicUrl} size={64} level="M" includeMargin={false} />
          <span className="text-[7px] font-black text-black mt-1 uppercase tracking-tight text-center">
      ...
      {/* Footer Info */}
      <div className="flex items-center justify-between border-t-2 border-black pt-0.5 mt-0.5 text-[8px] font-bold leading-none text-black">
